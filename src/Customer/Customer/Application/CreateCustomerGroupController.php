...
use CTIC\Customer\Customer\Infrastructure\Controller\CustomerGroupController;
use CTIC\Customer\Customer\Infrastructure\Form\Type\CustomerGroupType;
...
class CreateCustomerGroupController extends CreateResourceController
{
    /**
    ...
    ): void
    {
        ...
        $formRegistry->addType(new CustomerGroupType(CustomerGroup::class));
    }
        ...
        if(empty($command->metadata))
        {
            $command->metadata = self::getMetadata(
                ...
                array(
                    ...
                    'driver'        => '',
                    'templates'     => '',
                    'header'        => 'grupo de clientes',
                    'route'         => array(
                        'root'      => 'clientes_grupos',
                        'actions'   => array(
                            'list'  => 'listado',
                            'show'  => 'mostrar',
                            'create'    =>'crear',
                    ...
                    'classes'       => array(
                        'form'      => 'CTIC\Customer\Customer\Infrastructure\Form\Type\CustomerGroupType'
                    ),
                    'grid'          => array(
                        'actionGroups'  => array(
                            'bulk'  => array(
    ...
    public static function setDefaultCommand(ResourceControllerCommand $command): void
    {
        if(empty($command->newResourceFactory))
        {
            $command->newResourceFactory = new CreateCustomerGroup();
        }
        if(empty($command->resource))
        {
            $command->resource = CustomerGroup::class;
        }
        if(empty($command->command))
        {
            $command->command = new CustomerGroupCommand();
        }
    }

    public static function getResourceControllerClass(): string
    {
        return CustomerGroupController::class;
    }
