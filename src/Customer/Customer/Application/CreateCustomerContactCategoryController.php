...
use CTIC\Customer\Customer\Infrastructure\Controller\CustomerContactCategoryController;
use CTIC\Customer\Customer\Infrastructure\Form\Type\CustomerContactCategoryType;
...
class CreateCustomerContactCategoryController extends CreateResourceController
{
    /**
    ...
    ): void
    {
        ...
        $formRegistry->addType(new CustomerContactCategoryType(CustomerContactCategory::class));
    }
        ...
        if(empty($command->metadata))
        {
            $command->metadata = self::getMetadata(
                ...
                array(
                    ...
                    'driver'        => '',
                    'templates'     => '',
                    'header'        => 'categoría de contacto de clientes',
                    'route'         => array(
                        'root'      => 'clientes_categoriacontactos',
                        'actions'   => array(
                            'list'  => 'listado',
                            'show'  => 'mostrar',
                            'create'    =>'crear',
                    ...
                    'classes'       => array(
                        'form'      => 'CTIC\Customer\Customer\Infrastructure\Form\Type\CustomerContactCategoryType'
                    ),
                    'grid'          => array(
                        'actionGroups'  => array(
                            'bulk'  => array(
    ...
    public static function setDefaultCommand(ResourceControllerCommand $command): void
    {
        if(empty($command->newResourceFactory))
        {
            $command->newResourceFactory = new CreateCustomerContactCategory();
        }
        if(empty($command->resource))
        {
            $command->resource = CustomerContactCategory::class;
        }
        if(empty($command->command))
        {
            $command->command = new CustomerContactCategoryCommand();
        }
    }

    public static function getResourceControllerClass(): string
    {
        return CustomerContactCategoryController::class;
    }
