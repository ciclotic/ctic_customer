...
use CTIC\Customer\Customer\Infrastructure\Controller\CustomerObservationCategoryController;
use CTIC\Customer\Customer\Infrastructure\Form\Type\CustomerObservationCategoryType;
...
class CreateCustomerObservationCategoryController extends CreateResourceController
{
    /**
    ...
    ): void
    {
        ...
        $formRegistry->addType(new CustomerObservationCategoryType(CustomerObservationCategory::class));
    }
        ...
        if(empty($command->metadata))
        {
            $command->metadata = self::getMetadata(
                ...
                array(
                    ...
                    'driver'        => '',
                    'templates'     => '',
                    'header'        => 'categoría de observación de clientes',
                    'route'         => array(
                        'root'      => 'clientes_categoriaobservaciones',
                        'actions'   => array(
                            'list'  => 'listado',
                            'show'  => 'mostrar',
                            'create'    =>'crear',
                    ...
                    'classes'       => array(
                        'form'      => 'CTIC\Customer\Customer\Infrastructure\Form\Type\CustomerObservationCategoryType'
                    ),
                    'grid'          => array(
                        'actionGroups'  => array(
                            'bulk'  => array(
    ...
    public static function setDefaultCommand(ResourceControllerCommand $command): void
    {
        if(empty($command->newResourceFactory))
        {
            $command->newResourceFactory = new CreateCustomerObservationCategory();
        }
        if(empty($command->resource))
        {
            $command->resource = CustomerObservationCategory::class;
        }
        if(empty($command->command))
        {
            $command->command = new CustomerObservationCategoryCommand();
        }
    }

    public static function getResourceControllerClass(): string
    {
        return CustomerObservationCategoryController::class;
    }
